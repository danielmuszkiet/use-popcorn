...
function StarRating({
  ...
  const [rating, setRating] = useState(defaultRating > maxRating ? maxRating : defaultRating);
      ...
      <div style={starContainerStyle}>
        {Array.from({ length: maxRating }, (_, i) => (
          <Star
            ...
            onMEnter={() => setTempRating(i + 1)}
            onMLeave={() => setTempRating(0)}
            color={color}
            size={size}
          />
        ))}
...
function Star({ onRate, onMEnter, onMLeave, full, color, size }) {
  const starStyle = {
    display: "block",
    height: `${size}px`,
    width: `${size}px`,
